...
def _suggest_action(error_msg: str) -> str:
    ...
    if "check constraint" in msg or "23514" in msg:
        return "Tipo de archivo no reconocido en la BD (ya corregido). El próximo sync lo indexará automáticamente."
    if "57014" in msg or "canceling statement" in msg:
        return "Timeout de BD en archivo grande (límite aumentado a 10min). El próximo sync lo reintentará."
    if "corrupt" in msg or "unexpected eof" in msg or "truncat" in msg:
        return "Archivo corrupto o truncado en Drive. Vuelve a subir el archivo original."
    if "no coincide con la extensión" in msg or "posible archivo manipulado" in msg:
        return "Extensión incorrecta (ej: HTML guardado como PDF). Corrige el archivo en Drive."
    return "Error desconocido. Mira el detalle del error y reintenta."
